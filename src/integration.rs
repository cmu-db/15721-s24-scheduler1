...
mod tests {
    ...
    ctx.register_csv("orders", "src/example_data/orders.csv", CsvReadOptions::new()).await?;
    ctx.register_csv("prices", "src/example_data/prices.csv", CsvReadOptions::new()).await?;
    ...
    let mut fragments = parser::parse_into_fragments_wrapper(physical_plan).await;
    ...
    let left = physical_plan::execute_stream(fragments.remove(&1).unwrap().root.unwrap(), context)?;
    let context = ctx.state().task_ctx();
    let right = physical_plan::execute_stream(fragments.remove(&2).unwrap().root.unwrap(), context)?;
